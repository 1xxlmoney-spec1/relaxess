# Relaxess Cloud Audio Setup Guide

This document explains how to configure and update audio files for Relaxess without rebuilding the app.

## Architecture Overview

Relaxess uses a **cloud-based audio streaming system** with local caching:
...
export const CLOUD_AUDIO_TRACKS: Record<AudioTrack, CloudAudioTrack> = {
  relaxm1: {
    ...
    name: 'Relaxing Music',
    category: 'music',
    cloudUrl: 'https://relaxess-audio.s3.amazonaws.com/relaxm1.mp3',
    localCachePath: `${FileSystem.cacheDirectory}relaxess_relaxm1.mp3`,
...
1. Create an S3 bucket (e.g., `relaxess-audio`)
...
Relaxess supports these audio categories for future expansion:
...
2. Add to `CLOUD_AUDIO_TRACKS`:
   ...
   ocean: {
     ...
     localCachePath: `${FileSystem.cacheDirectory}relaxess_ocean.mp3`,
     duration: 15 * 60 * 1000,
     isPremium: true,
     size: 16 * 1024 * 1024,
   }
